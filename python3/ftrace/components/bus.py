...
class Bus(FTraceComponent):
    ...
    def bus_request_intervals(self, device=None, state=None, interval=None):
        ...
        if device is not None:
            ...
            intervals = IntervalList(sorted_items(list(self._bur_intervals_by_dev.values()))).slice(interval=interval)

        filter_func = (lambda bi: bi.state is state) if state else None
        return IntervalList(list(filter(filter_func, intervals)))
