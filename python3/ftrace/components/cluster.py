...
class Cluster(FTraceComponent):
    ...
    def busy_intervals(self, cluster=None, interval=None):
        ...
        """
        try:
            return IntervalList([cii for cii in self.cluster_intervals(cluster=cluster,
                                                          interval=interval) if cii.state!=BusyState.IDLE])
        except:
            return IntervalList()
    ...
    def cluster_intervals(self, cluster, interval=None):
        ...
        if cluster is not None:
            ...
            intervals = IntervalList(sorted_items(list(self._cluster_idle_intervals_by_cluster.values())))
