...
class Android(FTraceComponent):
    ...
    def event_intervals(self, name=None, task=None,
        ...
        if name is None:
            intervals = \
                IntervalList(sorted_items(list(self._tmw_intervals_by_name.values())))
        ...
        if task:
            intervals = IntervalList([it for it in intervals if it.event.task == task])
    ...
    def jank_intervals(self, interval=None):
        ...
        return IntervalList([x for x in missedFrames if x.value==1])
    ...
    def input_events(self, task=None, interval=None):
        ...
        return IntervalList([input_event for input_event in all_inputs if input_event.value==1])
